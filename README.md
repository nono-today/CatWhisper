...
[![Platform](https://img.shields.io/badge/Platform-macOS%2015+-black.svg)](https://www.apple.com/macos/)
...
### Hold fn, just talk
No window switching, no app to open. Hold the fn key in any application, speak, and the transcription is typed at your cursor when you release. Pick the Nemotron streaming model and text streams in live, as you speak.
...
# Build and run (requires xcodegen; xcodebuild is needed for Metal shaders)
...
| **Nemotron Streaming 0.6B** | ~600MB | 25 | Live dictation — text streams in as you speak (requires Accessibility) |
...
- [x] Real-time streaming transcription (Nemotron streaming model, v1.2.0)
...
### 按住 fn，說話就好
不需要切換視窗、不需要打開 App。在任何應用程式中按住 fn 鍵說話，放開後辨識結果自動輸入到游標位置。選用 Nemotron 串流模型還能邊講邊出字、即時輸入。
...
# 建置並執行（需要 xcodegen；Metal shader 編譯需透過 xcodebuild）
brew install xcodegen
xcodegen generate
./run.sh
...
| **Nemotron 串流 0.6B** | ~600MB | 25 | 即時聽寫 — 邊講邊出字（需輔助使用權限） |
